...
def e_node_selected(cluster_member, node_member, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, choose):
    if dead == 0:
        # Calculate all energy use to send/Receive pkt control
    ...
    with open('cluster_member.csv', 'w', newline='') as csvnew:
        ...
        for line1 in cluster_member:
            write.writerow(line1)
    if int(choose) != 99:
        with open('cluster_member_o.csv', 'w', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line2 in cluster_member:
                write.writerow(line2)
        with open('node_member_o.csv', 'w', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line4 in node_member:
...
def node_selected(cluster_member, node_member, r2, dead, choose):
    max_dis = []
    node_select = []
    log_c_select = []
    data_distance = []
    ...
    # append data to csv. file
    with open('node_select.csv', 'w', newline='') as csvnew:
        write = csv.writer(csvnew)
        for line1 in node_select:
            write.writerow(line1)
    if int(choose) != 99:
        with open('node_select_o.csv', 'w', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line2 in node_select:
                write.writerow(line2)
...
def back_to_nodes(cluster_member, node_member, max_dis, r1, t_predefine, choose, count_lap, dead_lap):
...
def back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine, count_lap, dead_lap):
    """ before next loop all cluster switch back to node_member """
    # collect data highest distance from each cluster
    data =[]
    for j in max_dis:
...
def start(choose):
    # print("choose 0 to reset values")
    ...
    if int(choose) == 0:
        dead = 0
        station_member, node_member_o, = [], []

        # collect bs in .csv
        station_member = \
            base_station(num_base, pos_base)

        # collect len_nodes in .csv
        ...
        cluster_member, node_member, dead = \
            ...
            pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, choose)
        
        
        data_distance, node_select, max_dis, log_c_select, dead = \
            node_selected(cluster_member, node_member, r2, dead, choose)
        
        lux_values = [[len_nodes, count_lap, dead]]
        with open('lux_values.csv', 'w', newline='') as lux_w:
        ...
        with open("cluster_member_o.csv", 'r') as csvnew:
            read = csv.reader(csvnew)
            for line3 in read:
                cluster_member.append(list(map(float, line3)))
        with open("node_select_o.csv", 'r') as csvnew:
            read = csv.reader(csvnew)
            for line4 in read:
                node_select.append(list(map(float, line4)))
        with open("lux_values.csv", 'r') as lux_r:
        ...
        # for i in cluster_member_o: print(i)
        # print("TYPE 1")
        ...
        while dead == 0:
            ...
            # for i in cluster_member: print(i)
            # print("TYPE 1 e_data_cluster")
        ...
        with open("cluster_member.csv", 'r') as csvnew:
            ...
            for line3 in read:
                cluster_member.append(list(map(float, line3)))
        with open("node_select.csv", 'r') as csvnew:
            read = csv.reader(csvnew)
            for line4 in read:
                node_select.append(list(map(float, line4)))
        with open("lux_values.csv", 'r') as lux_r:
        ...
        while dead == 0:
            cluster_member, node_member, dead = \
                ...
                pkt_data, elec_tran, elec_rec, fs, mpf, d_threshold, dead)
            # for i in cluster_member: print(i)
            # print("TYPE 2 e_data_cluster")

            cluster_member, station_member, dead = \
                e_cluster_bs(cluster_member, station_member, pkt_data, \
                elec_tran, elec_rec, fs, mpf, d_threshold, dead)
            # for i in cluster_member: print(i)
            # print("TYPE 2 e_cluster_bs")
            ...
            if count_lap == 35:
                ...
                with open('cluster_member.csv', 'w', newline='') as csvnew:
                    write = csv.writer(csvnew)
                    for line5 in cluster_member:
                        write.writerow(line5)
                with open('node_member.csv', 'w', newline='') as csvnew:
                    write = csv.writer(csvnew)
                    for line6 in node_member:
        ...
        count_lap += 1
        lux_v = [[len_nodes, count_lap, dead]]
        print(count_lap)
        with open('lux_values.csv', 'w', newline='') as lux_w:
            write = csv.writer(lux_w)
            for v in lux_v:
...
#     t1 = time.time()
#     p = pool.Pool()
#     result = p.imap(loop,range(1,201)) # doing loop function N laps with multi threads
#     time.sleep(0.4)
#     p.close()
#     p.join()
