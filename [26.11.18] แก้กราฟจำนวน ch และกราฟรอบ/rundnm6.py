...
def back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine, count_lap, dead_lap, dead, count_ch_member):
    ...
    if dead == 0:
        ...
        with open('data t dynamic 0.6 and r0.csv', 'a', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line1 in data:
                write.writerow(line1)
                
        len_ch = [[count_lap, len(cluster_member),count_ch_member ]]
        with open('data cluster dynamic 0.6.csv', 'a', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line in len_ch:
                write.writerow(line)
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    while True:
        ...
        if dead == 0:
            ...
            with open('count lap dynamic 0.6.csv', 'a', newline='') as csvnew:
                write = csv.writer(csvnew)
                for line in keep:
                    write.writerow(line)
            break
