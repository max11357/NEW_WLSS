...
def base_station(num_base, pos_base):
    ...
    with open('station_memberD3.csv', 'w', newline='') as csvnew:
...
def random_nodes(width, height, station_member, set_energy, density, t_predefine):
    ...
    while len(node_member_o) != len_nodes:
        ...
        count += 1
    # append data to csv. file
    with open('node_member_oD3.csv', 'w', newline='') as csvnew:
        write = csv.writer(csvnew)
        for line1 in node_member_o:
            write.writerow(line1)
    with open("len_nodesD3.txt", "w") as text_file:
...
def e_distance_candidate(cch, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, r1):
    ...
    if dead == 0:
        ...
        for main in range(len(cch)):
            waste_tran = pkt_control*((fs*(30**2))+elec_tran)
            waste = (elec_tran + (fs*(30**2)))*pkt_control # announce diastance r0
            ...
            for other in range(len(cch)):
                ...
                waste_rec = (elec_rec*pkt_control)
                    # Receive pkt control
                if distance <= 30:
                    if cch[other][2] - waste_rec > 0:
                        cch[other][2] = cch[other][2] - waste_rec
                    else:
                        dead = 1
...
def e_let_node_selected(cluster_member, node_member, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, r2):
    if dead == 0:
        ...
        for cluster in range(len(cluster_member)):
            waste = (elec_tran + (fs*(61**2)))*pkt_control# announce ch distance r2
            ...
            for node in range(len(node_member)):
                distance = math.sqrt((cluster_member[cluster][0] - node_member[node][0])**2 + \
                                     (cluster_member[cluster][1] - node_member[node][1])**2)
                waste_rec = (elec_rec*pkt_control)
                    # Receive pkt control
                if distance <= 61:
                    if node_member[node][2] - waste_rec > 0:
                        node_member[node][2] = node_member[node][2] - waste_rec
                    else:
                        dead = 1
    ...
    if dead == 0:
        ...
        for j in log_c_select:
            if j[0] != -1 and j[0] == check_c:
                ...
                check_c += 1
        cluster_select.append(log)
        # cluster_select = sorted(cluster_select)
        # if [] in cluster_member:
        #     cluster_select.remove([])
...
def e_cluster_bs(cluster_member, station_member, pkt_data, elec_tran, elec_rec, fs, mpf, \
    ...
    if dead == 0:
        ...
        for member in range(len(count_ch_member)):
            cluster_member[member][2] -= (count_ch_member[member]+1)*pkt_data* elec_tran
...
def e_optimize_t(cluster_member, node_member, node_select, max_dis, r1, pkt_data,\
               pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, r2, amount_n_with_c, cluster_select):
    if dead == 0:
        # loss energy from send and recieve data
        for distance in max_dis:
            # Send pkt data [node<--cluster]
            try:
                if  distance[1] < d_threshold :
                    e_tx = ((elec_tran + (fs*(distance[1]**2)))*pkt_control)
                    if cluster_member[distance[0]][2] - e_tx  > 0:
                        cluster_member[distance[0]][2] = cluster_member[distance[0]][2] - e_tx
                    else:
                        dead = 1
                else:
                    e_tx = ((elec_tran + (mpf*(distance[1]**4)))*pkt_control)
                    if cluster_member[distance[0]][2] - e_tx  > 0:
                        cluster_member[distance[0]][2] = cluster_member[distance[0]][2] - e_tx
                    else:
                        dead = 1
...
def back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine, count_lap, dead_lap, dead, count_ch_member):
    ...
    if dead == 0:
        ...
        try:
            ...
            print("")
            for h in cluster_member:print(h)
            print("")
            for i in max_dis:print(i)
            print("back_to_nodes_dynamic")
        with open('data t dynamic 0.3 and r0.csv', 'a', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line1 in data:
                write.writerow(line1)
                
        len_ch = [[count_lap, len(cluster_member),count_ch_member ]]
        with open('data cluster dynamic 0.3.csv', 'a', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line in len_ch:
                write.writerow(line)
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    with open("station_memberD3.csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line1 in read:
            station_member.append(list(map(int, line1)))
    with open("node_member_oD3.csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line2 in read:
            node_member_o.append(list(map(float, line2)))
    
    while True:
        with open("len_nodesD3.txt", "r") as text_file:
        ...
        cluster_member, node_member, dead = \
            ...
            mpf, d_threshold, dead, r2, amount_n_with_c, cluster_select)
      

        cluster_member, node_member, dead = \
            optimize_t(cluster_member, node_member, node_select, max_dis, \
            decimal, decrease_t, increase_t, r1, dead, cluster_select, amount_n_with_c)
        ...
        if dead == 0:
            ...
            with open('count lap dynamic 0.3.csv', 'a', newline='') as csvnew:
                write = csv.writer(csvnew)
                for line in keep:
                    write.writerow(line)
            break
