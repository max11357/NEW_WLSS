...
def optimize_t(cluster_member, node_member, node_select, max_dis, r1, pkt_data,\
    ...
    if dead == 0:
        ...
        for k in range(len(cluster_member)):
            print(max_dis[k][1], r1,  cluster_member[k][3] , 1 ,cluster_member[k][3] , 0)
...
def back_to_nodes(cluster_member, node_member, max_dis, r1, t_predefine, choose, count_lap, i):
    """ before next loop all cluster switch back to node_member """
    # collect data highest distance from each cluster
    if int(choose) != 99:
        data =[]
        for j in max_dis:
            data.append([i,count_lap,j[1], cluster_member[j[0]][3]])
...
def back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine, count_lap,i):
    """ before next loop all cluster switch back to node_member """
    data =[]
    for j in max_dis:
        data.append([i, count_lap, cluster_member[j[0]][3], j[1]])
            ...
            if int(choose) == 0:
                ...
                for t in set_t_value:
                    ...
                    while True:
                        ...
                        cluster_member_last, node_member = \
                            back_to_nodes(cluster_member, node_member, max_dis, r1, \
                            t_predefine, choose, count_lap,i)
                        count_lap += 1
                        if dead == 1:
                            print("LAP : "+ str(count_lap))
                            break;
