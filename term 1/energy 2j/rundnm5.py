...
def base_station(num_base, pos_base):
    ...
    with open('station_memberD5.csv', 'w', newline='') as csvnew:
...
def random_nodes(width, height, station_member, set_energy, density, t_predefine):
    ...
    while len(node_member_o) != len_nodes:
        ...
        count += 1
    # append data to csv. file
    with open('node_member_oD5.csv', 'w', newline='') as csvnew:
        write = csv.writer(csvnew)
        for line1 in node_member_o:
            write.writerow(line1)
    with open("len_nodesD5.txt", "w") as text_file:
...
def back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine, count_lap, dead_lap, dead, count_ch_member):
    ...
    if dead == 0:
        ...
        with open('data t dynamic 0.5 and r0.csv', 'a', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line1 in data:
                write.writerow(line1)
                
        len_ch = [[count_lap, len(cluster_member),count_ch_member ]]
        with open('data cluster dynamic 0.5.csv', 'a', newline='') as csvnew:
            write = csv.writer(csvnew)
            for line in len_ch:
                write.writerow(line)
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    with open("station_memberD5.csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line1 in read:
            station_member.append(list(map(int, line1)))
    with open("node_member_oD5.csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line2 in read:
            node_member_o.append(list(map(float, line2)))
    
    while True:
        with open("len_nodesD5.txt", "r") as text_file:
        ...
        if dead == 0:
            ...
            with open('count lap dynamic 0.5.csv', 'a', newline='') as csvnew:
                write = csv.writer(csvnew)
                for line in keep:
                    write.writerow(line)
            break
