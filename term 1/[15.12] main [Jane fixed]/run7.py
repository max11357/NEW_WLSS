...
def e_agg(cluster_head, count_ch_member, pkt_data, elec_tran, dead_point, dead, used_energy):
    # summarize data
    if dead == 0:
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    with open("bs_member"+str(t_value*10)+".csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line1 in read:
            bs_member.append(list(map(int, line1)))
    with open("cm_original"+str(t_value*10)+".csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line2 in read:
            cm_original.append(list(map(float, line2)))
    
    while True:
        with open("len_cm"+str(t_value*10)+".txt", "r") as text_file:
        ...
        if dead == 0:
            ...
            print("run"+str(t_value)+" DeadLAP : "+ str(count_lap-1))
