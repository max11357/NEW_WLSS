...
def check_data_cm(cluster_member, at1, at2, at3, at4, at5, at6, \
                at7, at8, at9, at10, at11, at12, at13, at14, at15, \
                at16, at17, at18, at19, t_value, cache_cm, collect_envi_cm, count_sr):
    send_or_not_cm = []
    check_super_round = 1
    ...
    if count_sr == 0:
        for cm in range(len(cluster_member)):
            send_or_not_cm.append([cluster_member[cm][5], 1])
            collect_envi_cm.append([cluster_member[cm][5], eval('at%d'% (cluster_member[cm][4]))[0][cache_cm]])
            # print(cluster_member[cm][5], cluster_member[cm][2])
            # print(cluster_member[cm][4], eval('at%d'% (cluster_member[cm][4]))[0][cache_cm])
            # print("expection 1")
        cache_cm += 1
        # print(send_or_not_cm)
        # print("----------------")
        # print(collect_envi_cm)
        # print(" ")

    else:
        # print("expection 2")
        for cm in range(len(cluster_member)):
            for cec in collect_envi_cm:
                if cluster_member[cm][5] == cec[0]:
                    old = float(cec[1])
                    new = float(eval('at%d'% (cluster_member[cm][4]))[0][cache_cm])
                    diff = abs(((old - new) / old)*100)
                    if diff > 2.0:
                        send_or_not_cm.append([cluster_member[cm][5], 1])
                        cec[1] = new
                        # print(cluster_member[cm][5], cluster_member[cm][2])
                        # print(cluster_member[cm][4], cec[1])
                        # print(cluster_member[cm][4], eval('at%d'% (cluster_member[cm][4]))[0][cache_cm])
                        # print(" ")
                    else:
                        send_or_not_cm.append([cluster_member[cm][5], 0])
                        # print(cluster_member[cm][5], cluster_member[cm][2])
        cache_cm += 1
        # print(send_or_not_cm)
        # print("----------------")
        # print(collect_envi_cm)
        # print(" ")
    print("at round", cache_cm)
    
    return cache_cm, check_super_round, send_or_not_cm, collect_envi_cm


def e_intra_sr(cluster_head, cluster_member, cm_select, pkt_data, elec_tran,\
                    elec_rec, fs, mpf, d_threshold, dead, dead_point, used_energy, send_or_not_cm):
    # print("CLUSTER MEMBER")
    # for i in cluster_member:
    #     print(i[5])
    # print(" ")
    # print("SEND OR NOT")
    # for j in send_or_not_cm:
    #     print(j[0])
    # print(" ")
    ...
    if dead == 0:
        ...
        for cm in range(len(cluster_member)):
            if cm_select[cm][1] > 0 and cluster_member[cm][5] == send_or_not_cm[cm][0] and send_or_not_cm[cm][1] == 1:
                # Send pkt data [cm-->ch]
                if  cm_select[cm][1] < d_threshold:
...
def check_data_ch(cluster_head, cluster_member, at1, at2, at3, at4, at5, at6, \
                at7, at8, at9, at10, at11, at12, at13, at14, at15, at16, at17, \
                at18, at19, collect_envi_cm, cache_ch, cm_select, collect_envi_ch, count_sr):
    send_or_not_ch = []
    check_super_round = 1
    sum_envi_ch = [ [] for _ in range(len(cluster_head))]
    
    for cm in range(len(cluster_member)):
        sum_envi_ch[cm_select[cm][0]].append(float(collect_envi_cm[cm][1]))
        # print(cluster_member[i][3:],"|", cm_select[i],"|", collect_envi_cm[i])
    # for i in sum_envi_ch: print(i)
    # print(" ")

    # print("CH", cache_ch)
    if count_sr == 0:
        for ch in range(len(cluster_head)):
            sum_envi_ch[ch].append(float(eval('at%d'% (cluster_head[ch][4]))[0][cache_ch]))
        
        for ch in range(len(cluster_head)): 
            send_or_not_ch.append([cluster_head[ch][5], 1])
            collect_envi_ch.append([cluster_head[ch][5], sum(sum_envi_ch[ch])/ float(len(sum_envi_ch[ch]))])
            # print(cluster_head[ch][5], cluster_head[ch][2])
            # print(sum(sum_envi_ch[ch])/ float(len(sum_envi_ch[ch])))
            # print(cluster_head[ch][5], sum(sum_envi_ch[ch])/ float(len(sum_envi_ch[ch])))
        cache_ch += 1
        # print(send_or_not_ch)
        # print("----------------")
        # print(collect_envi_ch)
        # print(" ")
            ...
            for cec in collect_envi_ch:
                if cluster_head[ch][5] == cec[0]:
                    ...
                    if diff > 2.0:
                        ...
                        send_or_not_ch.append([cluster_head[ch][5], 0])
                        # print(cluster_head[ch][5], cluster_head[ch][2])
        cache_ch += 1
        # print(send_or_not_ch)
        # print("----------------")
    ...
    return cache_ch, send_or_not_ch

# ทำ e_agg_sr, e_route_sr ด้วยยยยยยยยยยยยยยยยยยยยยยยยยยยยยย [CHECK]
# ทำ super roound [PROCESSING]

def e_agg_sr(cluster_head, count_ch_member, pkt_data, elec_tran, dead_point, dead,\
          used_energy, send_or_not_ch):
    # summarize data
    if dead == 0:
        for member in range(len(count_ch_member)):
            if cluster_head[member][5] == send_or_not_ch[member][0] and send_or_not_ch[member][1] == 1:
...
def e_route_sr(cluster_head, bs_member, pkt_data, elec_tran, elec_rec, fs, mpf, \
                  d_threshold, dead, dead_point, used_energy, send_or_not_ch):
    # UNICAST
    if dead == 0:
        base_x, base_y = zip(*bs_member)
        for ch in range(len(cluster_head)):
            # Send pkt data [clsuter-->bs]
            if cluster_head[ch][5] == send_or_not_ch[ch][0] and send_or_not_ch[ch][1] == 1:
...
def back_to_cm_dynamic(cluster_head, cluster_member, max_distance, r1, t_value, count_lap, \
                    ...
                    check_super_round, super_round, count_sr, check_optimize_t, cache_cm, \
                    cache_ch, collect_envi_cm, collect_envi_ch):
    ...
    if dead == 0:
        ...
        if check_super_round == 1 and count_sr == super_round:
            ...
            # cache_cm = 0
            # cache_ch = 0
            collect_envi_cm, collect_envi_ch = [], []
            ch_t_compare = []
    ...
    return cluster_member, count_sr, check_optimize_t, check_super_round, count_sr, cache_cm, cache_ch, collect_envi_cm, collect_envi_ch, ch_t_compare
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    while True:
        ...
        cache_cm, check_super_round, send_or_not_cm, collect_envi_cm = \
        check_data_cm(cluster_member, at1, at2, at3, at4, at5, at6, \
            at7, at8, at9, at10, at11, at12, at13, at14, at15, at16, at17, at18, \
            at19, t_value, cache_cm, collect_envi_cm, count_sr)
        ...
        cache_ch, send_or_not_ch = \
        check_data_ch(cluster_head, cluster_member, at1, at2, at3, at4, at5, at6, \
            at7, at8, at9, at10, at11, at12, at13, at14, at15, at16, at17, \
            at18, at19, collect_envi_cm, cache_ch, cm_select, collect_envi_ch, count_sr)
        ...
        cluster_member, count_sr, check_optimize_t, check_super_round, count_sr, cache_cm, \
        cache_ch, collect_envi_cm, collect_envi_ch, ch_t_compare = \
        back_to_cm_dynamic(cluster_head, cluster_member, max_distance, r1, t_value, \
            ...
            cache_cm, cache_ch, collect_envi_cm, collect_envi_ch)
