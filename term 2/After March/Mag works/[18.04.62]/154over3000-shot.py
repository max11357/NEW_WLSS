...
def base_bs(num_base, pos_base, t_value, super_round, diff_per):
    """input bs point"""
    # Set POS bs here
    bs_member = []
...
def random_cm(width, height, bs_member, set_energy, density, t_value, super_round, diff_per):
    """random cm"""
    # Random cm
    cm_original = []
...
def become_ch(cch, r1, cluster_member, dead, cm_original, super_round, diff_per):
    ...
    me_ch = []
    me_not = []
    me_ch.append(cch[0]) # cch ตัวที่ 0 ไม่ว่าจะ overlap ใครหรือไม่ก็ได้เป็นเสมอเพราะพลังงานมากสุด
    count = 0

    for main in range(1, len(cch)):
        for other in range(2, len(cch)):
            if cch[main] != cch[other]: # ทำให้ไม่เช็คตัวเอง
                mo_distance = math.sqrt((cch[main][0] - cch[other][0])**2 + (cch[main][1] - cch[other][1])**2)
                e_main, e_other = cch[main][2], cch[other][2] # ดึงค่า energy ของ main และ other มาเก็บในตัวแปร    
                
                if mo_distance <= r1: # main - other  overlap กัน
                    if e_main > e_other: # ถ้า e_main มากกว่า e_other

                        # เอา main ไปเช็คกับ(ตัวที่จะเป็น ch)
                        state = [] # เก็บสถานะการเปรียบเทียบกัน (1 ได้เป็น ch, 0 ไม่ได้เป็น ch)
                        for check in me_ch:
                            e_check =  check[2]
                            mc_distance = math.sqrt((cch[main][0] - check[0])**2 + (cch[main][1] - check[1])**2)
                            if check != cch[main]: # ทำให้ไม่เช็คตัวเอง
                                if mc_distance > r1 and e_check > e_main and cch[main] not in me_ch :
                                    state.append(1)
                                else:
                                    state.append(0)
                        if 0 not in state:
                            me_ch.append(cch[main])
                    elif e_main < e_other: # ถ้า e_other มากกว่า e_main

                        # เอา other ไปเช็คกับ(ตัวที่จะเป็น ch)
                        for check in me_ch:
                            e_check =  check[2]
                            oc_distance = math.sqrt((cch[other][0] - check[0])**2 + (cch[other][1] - check[1])**2)
                            if check != cch[other]: # ทำให้ไม่เช็คตัวเอง
                                if oc_distance > r1 and e_check > e_other and cch[other] not in me_ch :
                                    state.append(1)
                                else:
                                    state.append(0)
                        if 0 not in state:
                            me_ch.append(cch[other])
            ...
            for check in me_ch:
                ...
                oc_distance = math.sqrt((cch[other][0] - check[0])**2 + (cch[other][1] - check[1])**2)
                if check != cch[other]: # ทำให้ไม่เช็คตัวเอง
                    if oc_distance <= r1 and e_check < e_other: # lead of other 
                        if check in me_ch:
                            dele = check
                            add = cch[other]
                            break;
    ...
    for i in me_ch:
        for x in me_ch:
            dis = math.sqrt((i[0] - x[0])**2 + (i[1] - x[1])**2)
            if dis < 30 and i != x:
                if i[2] > x[2]:
                    dele = x
                elif i[2] < x[2]:
...
def comp(cch, r1, cluster_member, dead, cm_original, me_ch, me_not, len_cm, super_round, diff_per):
    cluster_head = []
    me_ch2 = []
    me_not2 = []
...
def e_dis(cluster_head, cluster_member, pkt_control, elec_tran, \
                elec_rec, fs, mpf, d_threshold, dead, r2, dead_point, used_energy, super_round, diff_per):
    # BROADCAST
    if dead == 0:
        # Calculate all energy use to send/Receive pkt control
...
##                    print(e_tx, elec_tran ,fs,r2**2,pkt_control)
...
def cm_choose_ch(cluster_head, cluster_member, r2, data_distance, dead, super_round, diff_per):
    cm_select = []
    log_cm_select = []
    amount_cm_in_ch = {}
...
def e_join(cluster_head, cluster_member, cm_select, pkt_control, elec_tran,\
                    elec_rec, fs, mpf, d_threshold, dead, dead_point, used_energy, super_round, diff_per):
    # UNICAST
    if dead == 0:
        pkt_control = 200 #in this loop pkt con is 4000
...
##                    print(e_tx ,elec_tran ,fs,cm_select[cm][1]**2,pkt_control)
...
def calculate_some_data(cluster_head, cm_select, log_cm_select, amount_cm_in_ch, dead, super_round, diff_per):
    max_distance = []
    ch_select = []
    count_ch_member = []
    cm_out_of_range = 0
...
def e_conf(cluster_head, cluster_member, pkt_control, elec_tran, \
           ...
           used_energy, super_round, diff_per):
    # UNICAST
    if dead == 0:
...
def check_data(cluster_head, cluster_member, at1, at2, at3, at4, at5, at6, \
                ...
                at16, at17, at18, at19, t_value, cache, collect_envi, cm_select, count_sr, super_round, diff_per):
    send_or_not = []
    check_super_round = 1
...
def e_intra_sr(cluster_head, cluster_member, cm_select, pkt_data, elec_tran,\
                    elec_rec, fs, mpf, d_threshold, dead, dead_point, used_energy, send_or_not, super_round, diff_per):
...
def e_agg_sr(cluster_head, count_ch_member, pkt_data, elec_tran, dead_point, dead,\
          used_energy, send_or_not, super_round, diff_per):
    # summarize data
    if dead == 0:
...
def e_route(cluster_head, bs_member, pkt_data, elec_tran, elec_rec, fs, mpf, \
                  d_threshold, dead, dead_point, used_energy, super_round, diff_per):
    # UNICAST
    if dead == 0:
        base_x, base_y = zip(*bs_member)
...
def optimize_t(cluster_head, cluster_member, cm_select, max_distance, decimal, \
                decrease_t, increase_t, r1, dead, check_optimize_t, ch_t_compare, super_round, diff_per):
...
def plot_graph(cluster_head, cluster_member, cch, bs_member, r1, r2, data_distance, super_round, diff_per):
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    bs_member = \
    base_bs(num_base, pos_base, t_value, super_round, diff_per)


    cm_original, len_cm = \
    random_cm(width, height, bs_member, set_energy, density, t_value, super_round, diff_per)
    ...
    while True:
        ...
        if check_super_round == 0:
            cch, cluster_member = \
            random_cch(cm_original, len_cm, count_lap, super_round, diff_per)
            ...
            me_ch, me_not = become_ch(cch, r1, cluster_member, dead, cm_original, super_round, diff_per)

            cluster_head, cluster_member, dead, cm_original = \
            comp(cch, r1, cluster_member, dead, cm_original, me_ch, me_not , len_cm, super_round, diff_per)
            
            
            cluster_head, cluster_member, dead, dead_point , used_energy= \
            e_dis(cluster_head, cluster_member, pkt_control, elec_tran, elec_rec, \
                fs, mpf, d_threshold, dead, r2, dead_point, used_energy, super_round, diff_per)


        cm_select, log_cm_select, data_distance, amount_cm_in_ch, dead = \
        cm_choose_ch(cluster_head, cluster_member, r2, data_distance, dead, super_round, diff_per)
        ...
        if check_super_round == 0:
            ...
            calculate_some_data(cluster_head, cm_select, log_cm_select, amount_cm_in_ch, dead, super_round, diff_per)
    

            cluster_head, cluster_member, dead, dead_point, used_energy = \
            e_conf(cluster_head, cluster_member, pkt_control, elec_tran, elec_rec, \
                fs, mpf, d_threshold, dead, dead_point, max_distance, used_energy, super_round, diff_per)
        ...
        check_data(cluster_head, cluster_member, at1, at2, at3, at4, at5, at6, \
            ...
            at19, t_value, cache, collect_envi, cm_select, count_sr, diff_per, super_round)
