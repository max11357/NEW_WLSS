...
def pull_data():
    ...
    # cache2 = 0
    # print("asd")
    # while cache2 != len(eval('at%d'% (i))[0]):
    #     print(eval('at%d'% (1))[0][cache2]," and ",'at%d'% (1))[0][cache2+1])
    # cache2 += 1
...
def random_cch(cm_original, len_cm,count_lap):
    ...
    if len(cch) == 0:
        random_cch(cluster_member, len_cm, count_lap)
...
def comp(cch, r1, cluster_member, dead, cm_original):
    ...
    # keep_cch = []
    # if dead == 0:
    #     for index in range(len(cch)):
    #         x,y = int(cch[index][0]), int(cch[index][1])
    #         check_eng.update({(x,y,index):cch[index][2]})
    #     keep = sorted(check_eng.items(), key=operator.itemgetter(-1))#min to max
    #     can_be.append(keep[-1])
    #     cluster_head.append(cch[keep[-1][0][2]])
    #     keep.remove(keep[-1])
        
    #     reduce = 1
    #     for ch in reversed(range(len(keep))):
    #         # print(ch)
    #         if ch >= int(len(keep)*reduce):
    #             cluster_member.append(cch[keep[ch][0][2]])
    #         elif ch < int(len(keep)*reduce):
    #             state  = [keep[ch][0][2]]
    #             for can in can_be:
    #                 distance2 = math.sqrt((int(keep[ch][0][0]) - int(can[0][0]))**2+\
    #                                    (int(keep[ch][0][1])- int(can[0][1]))**2)
    #                 if distance2 >= 30 and keep[ch] not in can_be:
    #                     state.append(1)
    #                 elif distance2 < 30:
    #                     state.append(0)
    #             if int(0) not in state :
    #                 cluster_head.append(cch[keep[ch][0][2]])
    #                 # print(cch[keep[ch][0][2]])
    #             elif 0 in state:
    #                 cluster_member.append(cch[keep[ch][0][2]])
...
def e_sink_sr(cluster_head, bs_member, pkt_data, elec_tran, elec_rec, fs, mpf, \
...
def e_sink(cluster_head, bs_member, pkt_data, elec_tran, elec_rec, fs, mpf, \
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    while True:
        ...
        e_sink_sr(cluster_head, bs_member, pkt_data, elec_tran, elec_rec, fs, mpf, \
            d_threshold, dead, dead_point, used_energy, send_or_not)
