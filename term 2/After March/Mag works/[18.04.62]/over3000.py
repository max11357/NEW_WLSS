...
def become_ch(cch, r1, cluster_member, dead, cm_original):
    ...
    for main in range(1, len(cch)):
        for other in range(2, len(cch)):
            ...
            if cch[main] != cch[other]:
                distance = math.sqrt((cch[main][0] - cch[other][0])**2 + (cch[main][1] - cch[other][1])**2)
                xmain ,  xother = cch[main][2],cch[other][2]                            
                if distance >= r1:
                    state = []
                    for check in me_ch:
                        ch_check =  check[2]
                        distance2 = math.sqrt((cch[main][0] - check[0])**2 + (cch[main][1] - check[1])**2)
                        if check !=cch[main]:
                            if distance2 < r1 and ch_check > xmain and  cch[main] not in me_ch:#b>c, b<a, c>a
                                    state.append(0)
                            elif distance2 >= r1 and  xmain > xother and cch[main] not in me_ch:
                                    state.append(1)
                            else:
                                state.append(0)
                    if 0 not in state:
                    ...
                    if xmain > xother:
                        ...
                        for check in me_ch:
                            ...
                            distance2 = math.sqrt((cch[main][0] - check[0])**2 + (cch[main][1] - check[1])**2)
                            if check !=cch[main]:
                                if distance2 < r1 and ch_check > xmain and  cch[main] not in me_ch:
                                    state.append(0)
                                elif distance2 >= r1 and ch_check > xmain and  cch[main] not in me_ch :
                                    state.append(1)
                                else:
                                    state.append(0)
                        if 0 not in state:
                        ...
                        for check in me_ch:
                            ch_check =  check[2]
                            distance3 = math.sqrt((cch[other][0] - check[0])**2 + (cch[other][1] - check[1])**2)
                            if check !=cch[main]:
                                if distance3 < r1 and ch_check > xother and  cch[other] not in me_ch:
                                    state.append(0)
                                elif distance3 >= r1 and ch_check > xother and  cch[other] not in me_ch :
                                    state.append(1)
                                else:
                                    state.append(0)
                        if 0 not in state:
            ...
            for check in me_ch:
                ...
                distance3 = math.sqrt((cch[other][0] - check[0])**2 + (cch[other][1] - check[1])**2)
                if check !=cch[main]:
                    if distance3 < r1 and ch_check < xother:
                        if check in me_ch:
                            dele = check
                            add = cch[other]
                            break;
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    while True:
        ...
        if check_super_round == 0:
            ...
            me_ch, me_not = become_ch(cch, r1, cluster_member, dead, cm_original)
