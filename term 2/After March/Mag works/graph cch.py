...
def read_cch():
    ...
    with open('dead point SR '+str(sr)+" "+str(diff)+'.csv', 'r') as csvnew:
        read = csv.reader(csvnew)
        for line in read:
            dead.append(list(map(float,line[:2])))
    with open('check ch SR '+str(sr)+" "+str(diff)+'.csv', 'r') as csvnew:
        read = csv.reader(csvnew)
        for line in read:
            if line != [' ']:
                cut = [int(line[0]),\
    ...
    with open('check cch SR '+str(sr)+" "+str(diff)+'.csv', 'r') as csvnew:
        read = csv.reader(csvnew)
        for line in read:
            if line != []:
