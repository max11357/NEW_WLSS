...
def SR(cluster_member, super_round,cluster_head, cm_select, pkt_data, elec_tran, \
                elec_rec, fs, mpf, d_threshold, dead, dead_point, used_energy , count_ch_member, bs_member):
    cluster_head = []
    if dead == 0:
##        print(1)
...
##        while super_round > 1:
        print(super_round+1)
        cluster_member = combine_cm(cluster_member)
##            cluster_head = combine_ch(cluster_head)
##            super_round -= 1
##                for cm in range(1,len(cluster_member)):
##                    old, new = float(cluster_member[cm][4][0]), float(cluster_member[cm][4][1])
##                    differ = (new- old)/ old*100
##    ##                print(cluster_member[cm][4])
##    ##                print(old, new)
##                    if abs(differ) > 2 :#difference more than 2 percent
##                        del(cluster_member[cm][4][0])
##                        cluster_head, cluster_member, dead, dead_point, used_energy = \
##                        e_intra(cluster_head, cluster_member, cm_select, pkt_data, elec_tran, \
##                        elec_rec, fs, mpf, d_threshold, dead, dead_point, used_energy)
...
def combine_cm(cluster_member):
    ...
    ##                print(old, new)
        if abs(differ) > 2 :#difference more than 2 percent
            ...
            print(' not chang', differ)
    return cluster_member

def combine_ch(cluster_head):
    for ch in cluster_head:
        print(ch)
...
##                mpf, d_threshold, dead, dead_point, used_energy)
        ...
        cluster_member= SR(cluster_member, super_round,cluster_head,cm_select, pkt_data, elec_tran, \
                elec_rec, fs, mpf, d_threshold, dead, dead_point, used_energy, count_ch_member, bs_member)
