...
def base_bs(num_base, pos_base):
    ...
    with open('bs_member3.csv', 'w', newline='') as csvnew:
...
def random_cm(width, height, bs_member, set_energy, density, t_value, decrease_val):
    ...
    while len(cm_original) != len_cm:
        ...
        count += 1
    # append data to csv. file
    with open('cm_original3.csv', 'w', newline='') as csvnew:
        write = csv.writer(csvnew)
        for line1 in cm_original:
            write.writerow(line1)
    with open("len_cm3.txt", "w") as text_file:
...
def e_comp(cch, pkt_control, elec_tran, elec_rec, fs, \
                         mpf, d_threshold, dead, r1, dead_point):
    # BROADCAST
    if dead == 0:
                ...
                for cm in range(len(cluster_member)):
                    ...
                    if distance <= r2:
                        if cluster_member[cm][2] - e_rx > 0:
                            ...
                            dead_point.append('cm e_dis recive each')
                            dead = 1
        ...
        for cm in range(len(cluster_member)):
            if cm_select[cm][1] > 0:
                ...
                if cluster_head[cm_select[cm][0]][2] - e_rx > 0:
                    ...
                    dead_point = cluster_member[cm]
                    dead_point.append('ch e_join recieve')
                    dead = 1
                ...
                for cm in range(len(cluster_member)):
                    ...
                    if distance <= max_distance[ch][1]:
                        if cluster_member[cm][2] - e_rx > 0:
                            ...
                            dead_point = cluster_member[cm]
                            dead_point.append('cm e_conf recive each')
                            dead = 1
...
def back_to_cm(cluster_head, cluster_member, max_distance, r1, t_value, count_lap, dead_lap, dead, count_ch_member, len_cm, cm_out_of_range):
...
def start(width, height, density, num_base, pos_base, set_energy, pkt_control, pkt_data, \
    ...
    with open("bs_member3.csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line1 in read:
            bs_member.append(list(map(int, line1)))
    with open("cm_original3.csv", 'r') as csvnew:
        read = csv.reader(csvnew)
        for line2 in read:
            cm_original.append(list(map(float, line2)))
    
    while True:
        with open("len_cm3.txt", "r") as text_file:
        ...
        dead, cch, dead_point = \
        e_comp(cch, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, r1, dead_point)
        ...
        e_dis(cluster_head, cluster_member, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, r2, dead_point)
        ...
        cluster_head, cluster_member, dead, dead_point = \
        e_join(cluster_head, cluster_member, cm_select, pkt_data, elec_tran, elec_rec, fs, mpf, d_threshold, dead, dead_point)
        ...
        cluster_head, cluster_member, dead, dead_point = \
        e_conf(cluster_head, cluster_member, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, dead_point, max_distance)


        cluster_head, cluster_member, dead, dead_point = \
        e_intra(cluster_head, cluster_member, cm_select, pkt_data, elec_tran, elec_rec, fs, mpf, d_threshold, dead, dead_point)
        

        cluster_head, dead, dead_point = \
        e_agg(cluster_head, count_ch_member, pkt_data, elec_tran, dead_point, dead)
        

        cluster_head, bs_member, dead, dead_point = \
        e_route(cluster_head, bs_member, pkt_data, elec_tran, elec_rec, fs, mpf, d_threshold, dead, dead_point)
        ...
        cluster_member = \
        back_to_cm(cluster_head, cluster_member, max_distance, r1, t_value, count_lap, dead_lap, dead, count_ch_member, len_cm, cm_out_of_range)
        
        
        if dead == 0:
            count_lap += 1
        elif dead == 1:
            print("run3 DeadLAP : "+ str(count_lap-1))
