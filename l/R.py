...
def e_distance_candidate(cch, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, broadcast):
    
    if dead == 0:
        # Calculate all energy use to send/Receive pkt control
        for main in range(len(cch)):
            for other in range(len(cch)):
                waste = ((elec_tran + (mpf*(broadcast**4)))*pkt_control)
...
def e_node_selected(cluster_member, node_member, pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, broadcast):
...
def plot_graph(cluster_member, node_member, cch, station_member, r1, r2, data_distance):
    ...
    plt.hist(data_distance ,bins = [0,5,10,15,20,25,30,35,40,45,50,55,60,65,70,75])
...
def back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine, count_lap):
    """ before next loop all cluster switch back to node_member """
    # collect data highest distance from each cluster
    data =[]
    for j in max_dis:
        data.append([count_lap,cluster_member[j[0]][3], j[1]])
...
def start():
    ...
    print("Choose 2 loop N fixed T-value")
    ...
    num_base = 1
    pos_base = "0,0"
    set_energy = 2 # set energy = 2 Joule
    pkt_control = 200 # bit
    pkt_data = 4000  # bit
            ...
            if int(choose) == 0:
                ...
                while True:
                    ...
                    dead, cch = \
                        ...
                        elec_rec, fs, mpf, d_threshold, dead, broadcast)
                    ...
                    cluster_member, node_member, dead = \
                        ...
                        pkt_control, elec_tran, elec_rec, fs, mpf, d_threshold, dead, broadcast)
                ...
                while True:
                    ...
                    cluster_member_last, node_member = \
                        back_to_nodes_dynamic(cluster_member, node_member, max_dis, r1, t_predefine,count_lap)
