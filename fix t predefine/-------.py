...
def data_fix(fix):
    ...
    print(fix[0][3],fix[201][3])
    for i in range(len(fix)-1):
        if fix[i][0] == fix[i+1][0] :
            if fix[i][3] != fix[i+1][3]:
                data_plot.append([fix[i][3], fix[i][1], ch_count])
                ch_count = 0
            count += 1
            f_count += 1
            dis_sum += fix[i][2]
...
def data_dynamic(t_dynamic):
    ...
    return dis_dynamic, t_dy  
    
    
def plot(fix_t_1, fix_t_2, fix_t_3, fix_t_4, fix_t_5, \
         fix_t_6, fix_t_7, fix_t_8, fix_t_9, dis_dynamic, t_dy):
    ...
    plt.plot(t_dy,label='dynamic averge is' +str("%.3f"%float(sum(t_dy)/len(t_dy))))
    plt.xlabel('round')
...
def read():
    ...
    t_dynamic = dynamic()
    dis_dynamic, t_dy = data_dynamic(t_dynamic)
