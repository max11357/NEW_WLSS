...
def nodes_select(cluster_member, node_member, pkt_control, elec_tran,\
                 elec_rec, fs, mpf, d_threshold, r2, data_distance, dead, keep_dis_r0):
    if dead == 0:
        # Calculate all energy use to send/Receive pkt control
...
##            print(log_max)
            keep_dis_r0.append(log_max)
        print(max_dis)
    return node_select, cluster_member, node_member, dead, data_distance, max_dis, keep_dis_r0
...
def plot_graph(cluster_member, node_member, cch, station_member, r1, r2, data_distance,
               keep_dis_r0 ,t_predefine, choose):
    ...
    plt.close()
    bins = np.linspace(math.ceil(min(keep_dis_r0)), math.floor(max(keep_dis_r0)), 20) # fixed number of bins

    plt.xlim([min(keep_dis_r0)-5, max(keep_dis_r0)+5])

    plt.hist(keep_dis_r0, bins=bins, alpha=0.5)
    plt.title('Random Gaussian data (fixed number of bins)')
    plt.xlabel('variable X (20 evenly spaced bins)')
    plt.ylabel('count')
    if choose == 2:
        plt.savefig('r0 dynamic.png')
    else:
        plt.savefig('r0 '+str(t_predefine)+'.png')
def start():
    ...
    if choose == 0:
        ...
        while True:
            ...
            node_select, cluster_member, node_member, dead, data_distance, max_dis, keep_dis_r0 = \
                nodes_select(cluster_member, node_member, pkt_control, \
                             elec_tran, elec_rec, fs, mpf, d_threshold,\
                             r2, data_distance , dead, keep_dis_r0)
            ...
            plot_graph(cluster_member, node_member, cch, station_member, r1,r2,\
                       data_distance, keep_dis_r0, t_predefine, choose)
